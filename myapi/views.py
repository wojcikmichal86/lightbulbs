...
def LightbulbList(request):
    ...
    serializer = DeviceSerializer(devices, many=True) #turns the data from the model to json
...
def LightbulbDetail(request, pk):
    ...
    serializer = DeviceSerializer(devices, many=False) #turns the data from the model to json
    return Response(serializer.data) #sends the json as a respons to the request


@api_view(['POST'])
def LightbulbCreate(request):
    serializer = DeviceSerializer(data=request.data) #turns the data from the request to json
...
def LightbulbUpdate(request, pk):
    ...
    serializer = DeviceSerializer(instance=device, data=request.data) #turns the data from the request to json
    if serializer.is_valid(): #validates the data from the request
        serializer.save() #saves the object in the DB with the request json data
    return Response(serializer.data) #sends the json as a respons to the request
...
def BlindsList(request):
    devices = Blinds.objects.all() #gets all the objects from the Lightbulb model
...
def BlindsDetail(request, pk):
    devices = Blinds.objects.get(id=pk) #gets an object from the Lightbulb model by id
...
def BlindsUpdate(request, pk):
    device = Blinds.objects.get(id=pk) #gets an object from the Lightbulb model by id
...
def TemperatureList(request):
    devices = Temperature.objects.all() #gets all the objects from the Lightbulb model
...
def TemperatureDetail(request, pk):
    devices = Temperature.objects.get(id=pk) #gets an object from the Lightbulb model by id
...
def TemperatureUpdate(request, pk):
    device = Temperature.objects.get(id=pk) #gets an object from the Lightbulb model by id
